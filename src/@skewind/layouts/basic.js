...
const scrollToActiveSidebar = () => {
  ...
  const sidebarNavLevelItemActive = sidebarNavScroll.querySelector(
    ".navigation-level__item--active",
  );
  const sidebarNavItemActive = sidebarNavScroll.querySelector(
  ...
  const calculateActiveScrollPosition = () => {
    ...
    return (
      ...
      sidebarNavScroll.getBoundingClientRect().top -
      sidebarNavScroll.offsetHeight / 2 -
      nav.offsetHeight / 2
    );
  };

  const doScroll = () => {
    const sidebarNavScrollContainer = sidebarNavScroll.querySelector(
...
const setCustomScrollForNotifAndMsg = () => {
  const messagesDropdownToggle = document.getElementById(
    "messagesDropdownToggle",
  );
  Dropdown.getInstance(messagesDropdownToggle)?.has(Scrollbar);

  const notificationsDropdownToggle = document.getElementById(
