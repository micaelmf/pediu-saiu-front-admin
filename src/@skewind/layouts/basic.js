...
const scrollToActiveSidebar = () => {
  ...
  const doScroll = () => {
    ...
    if (sidebarNavScrollContainer.scrollTo) {
      sidebarNavScrollContainer.scrollTo({ top: calculateActiveScrollPosition(), behavior: 'smooth' })
      return
    }
  }
  ...
  if (collapseEl) {
    ...
    collapseEl.addEventListener('collapse.shown', () => {
      doScroll()
    }, { once: true })
  } else {
    doScroll()
