...
if (!args.output && !args.o) {
  throw new Error('Please specify the output path by using `--output` or `-o` argument.');
}

const host = args.host ?? args.h ?? 'http://localhost:5173';

const outputPath = path.resolve(path.join.apply(null, (args.output ? args.output : args.o).split('/')));
