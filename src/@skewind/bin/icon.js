...
if (!args.path && !args.p) {
  throw new Error('Please specify path to icons directory with `--path` or `-p` argument.');
}

const iconPath = path.resolve(path.join.apply(null, (args.p ? args.p : args.path).split('/')));
...
Object.keys(config).forEach((vendor) => {
  ...
  Object.keys(icons).forEach((icon) => {
    fs.appendFileSync(examplePath, `  // ${camelCase(vendor)}__${camelCase(icon)},\n`);
    fs.appendFileSync(buildPath, `export const ${camelCase(vendor)}__${camelCase(icon)} = \`${icons[icon]}\`;\n`);
    combinedIcons[`${camelCase(vendor)}__${camelCase(icon)}`] = icons[icon];
  });
});

fs.writeFileSync(path.join(iconPath, 'icons.json'), JSON.stringify(combinedIcons));
