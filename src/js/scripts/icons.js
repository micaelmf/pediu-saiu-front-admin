...
const buildItems = (icons) => {
  return Object.keys(icons).map((name) => {
    const element = crel('a', { href: 'javascript:;', class: 'flex flex-col items-center justify-center gap-5 rounded bg-gray-50 p-5 shadow text-xs transform hover:scale-105 relative' })
    ...
    element.addEventListener('click', async () => {
      const copiedEl = crel('span', { class: 'absolute inset-0 flex items-center justify-center bg-gray-50 text-base font-semibold' }, 'Copied!')
